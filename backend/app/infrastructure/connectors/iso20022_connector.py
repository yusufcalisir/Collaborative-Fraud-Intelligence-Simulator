...
class ISO20022MessagingConnector(BaseBankConnector):
    ...
    def parse_pacs008_xml(self, xml_content: str) -> NormalizedTransaction:
        ...
        msg_id = root.findtext(".//GrpHdr/MsgId") or f"pacs008_{int(datetime.utcnow().timestamp())}"
        ...
        currency = amount_elem.get("Ccy") if amount_elem is not None else "EUR"
    ...
    def parse_swift_mt103(self, mt103_text: str) -> NormalizedTransaction:
        ...
        tx_id = f"MT103_{int(datetime.utcnow().timestamp())}"
        amount = 500.0
        currency = "USD"
        debtor = "SWIFT_DEBTOR"
