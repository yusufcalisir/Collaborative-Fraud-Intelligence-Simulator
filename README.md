...
| **Public AML Dataset Benchmarks** | `dataloader.py` integrates loaders for the canonical **Elliptic Bitcoin**, **AMLSim**, and **PaySim/Kaggle CC** datasets. | Establishes academic credibility and validates performance on real-world transaction graphs. Runs on realistic synthetic mocks if real files are missing. | Fully offline development/CI capability with high-fidelity mock fallbacks |
| **FedYogi Server Optimizer** | Server-side adaptive optimizer using Yogi second-moment update $v_{t+1} = v_t - (1 - \beta_2) \cdot \text{sign}(v_t - \Delta_t^2) \cdot \Delta_t^2$. | Stabilizes convergence and mitigates gradient staleness/sparsity on highly imbalanced fraud labels. | Adaptive learning rate control with slow variance decay |
| **SCAFFOLD Convergence** | Control-variate based gradient correction at client-side: $g_i \leftarrow g_i - c_i + c$. | Mitigates client drift in heterogeneous (Non-IID) data distributions. | $\mathcal{O}(1)$ drift reduction guarantees |
