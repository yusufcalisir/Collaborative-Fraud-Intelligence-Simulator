...
| Feature | Technical Implementation | Purpose / Advantage | Cryptographic / ML Guarantee |
| :--- | :--- | :--- | :--- |
| **Trimmed Mean Aggregation** | Coordinate-wise removal of top/bottom $f$ updates before averaging; configurable Byzantine fraction. | Defends against outlier weight injections without requiring a trust model. | Breakdown point proportional to $f/N$ |
| **Bulyan Aggregation** | Nested selection: Krum pre-selects $N{-}2f$ candidates; coordinate-wise Trimmed Mean is applied over the selected set. | Neutralizes colluding Byzantine nodes that evade Krum or Median individually. | Tolerates up to $f$ colluders where $N \ge 4f+3$ |
| **MIA Evaluator** | Shadow model Membership Inference Attack via train/test loss threshold decision boundary. | Audits whether the global model leaks membership of individual training samples. | Empirical $\epsilon$-DP proxy via attack advantage score |
